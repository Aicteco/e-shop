...
@Component({
  selector: 'app-clotheswoman-list',
  templateUrl: './clotheswoman-list.component.html',
  styleUrls: ['./clotheswoman-list.component.scss']
})
export class ClotheswomanListComponent implements OnInit {
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    this.supplierServices.getAll( '1' ,'300', '0', '3','').subscribe(res => {
  ...
  navigateToProduct(id: number) {
    this.route.navigate([`./shoping/clotheswomen/product/${id}`]);
  }
  ...
  selectCity(citys:City){
    ...
    this.supplierServices.getAll( '1', '300' , String(this.city.id) , '3' ,'').subscribe(res => {
      this.supplierList = res.result.map(item => {
        item.imagePath = 'http://ahmedinara00-001-site1.dtempurl.com/' + item.imagePath;
        return item
    })
