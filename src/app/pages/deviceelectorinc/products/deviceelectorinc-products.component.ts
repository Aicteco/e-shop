...
@Component({
  selector: 'app-deviceelectorinc-products',
  templateUrl: './deviceelectorinc-products.component.html',
  styleUrls: ['./deviceelectorinc-products.component.scss']
})
export class DeviceelectorincProductsComponent implements OnInit {
 ...
 private spinner:NgxSpinnerService,
  ...
  getSlide(id){String(id)
    this.spinner.show();
   this.productServices.getAll('1','900', String(id),'9','').subscribe(res =>{
  ...
  postCart() {
    ...
    if(localStorage.token == null) {
      ...
      this.toastr.warning('تسجيل الدخول اولا ');
    ...
    this.cartServices.creatCart(this.cart).subscribe(
      res=>{
        ...
        this.toastr.success('تم اضافة المنتج الي العربة ');
      }
    )
  }
