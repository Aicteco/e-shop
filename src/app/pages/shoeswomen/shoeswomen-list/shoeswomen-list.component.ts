...
@Component({
  selector: 'app-shoeswomen-list',
  templateUrl: './shoeswomen-list.component.html',
  styleUrls: ['./shoeswomen-list.component.scss']
})
export class ShoeswomenListComponent implements OnInit {
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    this.supplierServices.getAll( '1' ,'300', '0', '6','').subscribe(res => {
  ...
  selectCity(citys:City){
    ...
    this.supplierServices.getAll( '1', '300' , String(this.city.id) , '6' ,'').subscribe(res => {
      this.supplierList = res.result.map(item => {
        item.imagePath = 'http://ahmedinara00-001-site1.dtempurl.com/' + item.imagePath;
        return item
    })
