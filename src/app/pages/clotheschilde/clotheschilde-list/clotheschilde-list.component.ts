...
@Component({
  selector: 'app-clotheschilde-list',
  templateUrl: './clotheschilde-list.component.html',
  styleUrls: ['./clotheschilde-list.component.scss']
})
export class ClotheschildListComponent implements OnInit {
...
term:string;
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    this.supplierServices.getAll( '1' ,'300', '0', '4','').subscribe(res => {
  ...
  navigateToProduct(id: number) {
    this.route.navigate([`./shoping/clotheschild/product/${id}`]);
  }
  ...
  selectCity(citys:City){
    ...
    this.supplierServices.getAll( '1', '300' , String(this.city.id) , '4' ,'').subscribe(res => {
      this.supplierList = res.result.map(item => {
        item.imagePath = 'http://ahmedinara00-001-site1.dtempurl.com/' + item.imagePath;
        return item
    })
