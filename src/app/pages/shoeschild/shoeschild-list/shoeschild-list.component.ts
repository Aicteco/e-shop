...
@Component({
  selector: 'app-shoeschild-list',
  templateUrl: './shoeschild-list.component.html',
  styleUrls: ['./shoeschild-list.component.scss']
})
export class ShoeschildListComponent implements OnInit {
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    this.supplierServices.getAll( '1' ,'300', '0', '7','').subscribe(res => {
  ...
  navigateToProduct(id: number) {
    this.route.navigate([`./shoping/shoeschild/product/${id}`]);
  }
  ...
  selectCity(citys:City){
    ...
    this.supplierServices.getAll( '1', '300' , String(this.city.id) , '7' ,'').subscribe(res => {
      this.supplierList = res.result.map(item => {
        item.imagePath = 'http://ahmedinara00-001-site1.dtempurl.com/' + item.imagePath;
        return item
    })
