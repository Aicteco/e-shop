...
export class MobilesListComponent implements OnInit {
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    this.supplierServices.getAll( '1' ,'300', '0', '8','').subscribe(res => {
  ...
  navigateToProduct(id: number) {
    this.route.navigate([`./shoping/mobiles/product/${id}`]);
  }
  ...
  selectCity(citys:City){
    ...
    this.supplierServices.getAll( '1', '300' , String(this.city.id) , '8' ,'').subscribe(res => {
      this.supplierList = res.result.map(item => {
        item.imagePath = 'http://ahmedinara00-001-site1.dtempurl.com/' + item.imagePath;
        return item
    })
