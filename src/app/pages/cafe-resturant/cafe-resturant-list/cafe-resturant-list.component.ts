...
@Component({
  selector: 'app-cafe-resturant-list',
  templateUrl: './cafe-resturant-list.component.html',
  styleUrls: ['./cafe-resturant-list.component.scss']
})
export class CafeResturantListComponent implements OnInit {
  ...
  constructor(
   ...
   ngOnInit(): void {
    ...
    this.supplierServices.getAll( '1' ,'300', '0', '12','').subscribe(res => {
  ...
  selectCity(citys:City){
    ...
    this.supplierServices.getAll( '1', '300' , String(this.city.id) , '12' ,'').subscribe(res => {
      this.supplierList = res.result.map(item => {
        item.imagePath = 'http://ahmedinara00-001-site1.dtempurl.com/' + item.imagePath;
        return item
    })
  ...
  navigateToProduct(id: number) {
    this.route.navigate([`./shoping/cafe/product/${id}`]);
  }
