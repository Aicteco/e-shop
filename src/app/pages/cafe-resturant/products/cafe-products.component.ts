...
import { proudctDetails } from '../../../shared/models/productdetails.model';
import { ToastrService } from 'ngx-toastr';


@Component({
  selector: 'app-cafe-products',
  templateUrl: './cafe-products.component.html',
  styleUrls: ['./cafe-products.component.scss']
})
export class CafeProductsComponent implements OnInit {
...
proudctlist:Products;
  ...
  getSlide(id){
    this.spinner.show();
    this.productServices.getAll('1','900', String(id),'12','').subscribe(res =>{
        this.proudct = res.result.map(item =>{
          item.imagePath = this.Url+item.imagePath;
          this.spinner.hide();
          return item
        })
  ...
  postCart() {
    ...
    if(localStorage.token == null) {
      ...
      this.toster.error('تسجيل الدخول اولا  '  , '')
    ...
    this.cartServices.creatCart(this.cart).subscribe(
      res=>{
        ...
        this.toster.success('شكرا لك', 'تم اضافه المنتج الي العربه ')
      }
    )
  }
